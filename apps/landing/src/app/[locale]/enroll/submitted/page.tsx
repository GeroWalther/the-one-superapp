    ...
    >
      <div className="glass edge-accent rounded-[26px] px-7 py-9 text-center sm:px-9">
        <span className="mx-auto grid h-14 w-14 place-items-center rounded-full border border-aqua-500/30 bg-aqua-500/10">
          {approved ? (
            <CheckCircle2 className="h-6 w-6 text-aqua-500" strokeWidth={1.5} />
          ) : (
            <Mail className="h-6 w-6 text-aqua-500" strokeWidth={1.5} />
          )}
        </span>
        ...
        <ol className="mx-auto mt-8 max-w-sm space-y-4 text-left">
          {steps.map((step, index) => (
            <li key={step} className="flex items-start gap-3">
              <span className="mt-[1px] grid h-6 w-6 shrink-0 place-items-center rounded-full border border-line bg-paper-soft text-[11px] font-semibold text-aqua-500">
                {index + 1}
              </span>
              <span className="text-[13.5px] leading-[1.65] text-ink-soft">
                {step}
              </span>
            </li>
          ))}
        ...
        <hr className="rule-accent mx-auto my-8 w-full max-w-xs" />

        <p className="text-[12.5px] leading-[1.7] text-ink-faint">
